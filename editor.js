...
function init() {
  ...
  document
    ...
    .addEventListener('click', openMetaModal)
...
async function exportBx2() {
  ...
  if (hasEffects || hasMeta) {
    ...
    if (state.meta.bpm !== '')
      metaObj.bpm = parseFloat(state.meta.bpm) || state.meta.bpm
    if (state.meta.related_media)
      metaObj.related_media = state.meta.related_media
  ...
  const content =
    hasEffects || hasMeta
      ? JSON.stringify(
          { meta: metaObj, markers: markerData, effects: state.effects },
          null,
          2,
        )
...
function onTlMouseMove(e) {
  ...
  if (tlMouse.draggingMarkerIdx >= 0 && tlMouse.down) {
    ...
    if (!tlMouse.dragMoved && Math.abs(x - tlMouse.dragStartX) < DRAG_THRESHOLD)
      return
    ...
    if (state.markers.some((mk, i) => i !== idx && mk.frame === newFrame))
      return
...
async function saveMetaModal() {
  ...
  if (_openFileHandle) {
    try {
      ...
      if (metaObj) {
        if (state.meta.title) metaObj.title = state.meta.title
        if (state.meta.path_creator)
          metaObj.path_creator = state.meta.path_creator
        if (state.meta.bpm !== '')
          metaObj.bpm = parseFloat(state.meta.bpm) || state.meta.bpm
        if (state.meta.related_media)
          metaObj.related_media = state.meta.related_media
